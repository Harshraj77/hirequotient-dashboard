...
const handleSelectAll = () => {
  ...
  const handleDeleteSelected = () => {
    ...
    const allIds = users.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage).map(user => user.id);
    const allSelected = selectedRows.length === allIds.length && allIds.every(id => selectedRows.includes(id));
    ...
    <div className="mt-4 px-8">
      ...
      {showPopup && (
        <div className="fixed top-0 left-0 w-full h-full bg-gray-500 bg-opacity-50 flex items-center justify-center">
          <PopupForm data={popupData} handleSave={handleSave} handleClose={() => setShowPopup(false)} />
        </div>
      )}
    </div>
  );
